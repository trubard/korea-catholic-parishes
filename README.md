...
| `audience` | 대상(리스트, 감지 시): `어린이·유아·초등부·고등부·중고등부·주일학교·학생·청소년·청년·장년·가족` 등 |
| `audience_raw` | 어휘에 없는 대상성 어구 원문 보존(감지 시): 열거 불가한 언어·국적 공동체 등(예: `동티모르공동체`·`스페인어`). 소비 쪽은 모르는 값도 그대로 표시하면 됩니다 |
| `kind` | 전례 성격(리스트, 감지 시): `교중·새벽·특전·신심·성시간·위령·연도` 등 |
| `type` | `audience`+`kind` 를 합친 하위호환 값 |
| `feast` | 그날의 축일 이름. **수집일 한정 관측값** — 아래 주의 |
| `seasonal` | 계절별 대안 시각(감지 시): `{"winter":"07:00"}` — 같은 미사의 다른 시각 |
...
> `audience`·`kind`·`feast`·`seasonal`·`place`·`recurrence`는 감지된 경우에만 존재합니다.
> `raw`(원문)는 항상 보존되므로 미분류 정보도 손실되지 않습니다.
>
> ⚠️ **`feast` 는 수집일 한정 관측값입니다.** 성당 홈페이지는 축일 주간에만 안내를
> 띄우므로, 이 필드는 "수집일이 축일이었을 때 그날 무슨 미사가 있었나"만 담습니다.
> "이 성당의 축일 미사가 언제인가"를 답하지 못하며, 다음 해엔 비어 있을 수 있습니다.
> 축일 판정은 이 필드에 의존하지 마세요.
>
> ⚠️ **`place` 가 있으면 공소 미사입니다.** 좌표·주소는 본당의 것이므로, 본당에서
> 봉헌되는 미사로 표시하면 안 됩니다(사용자를 다른 건물로 안내하게 됩니다).
